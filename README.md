...
hiv-run exists to test hiv-agent and its use of the **sierrapy** skill under realistic conditions. The agent works here with only a user prompt (`PROMPT.txt`), input data (`data/`), and the sierrapy skill (`.cursor/skills/sierrapy/`). It does not see grading artifacts from hiv-agent, such as the rubric or evaluation markdowns. If those were present, the agent could tailor its behavior to the rubric and fabricate skill usage, making the test invalid.
